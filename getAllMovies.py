...
request = requests.get(url="http://movie.douban.com/j/search_tags?type=movie")
tags = json.loads(request.text)['tags']
# print(tags)
...
for tag in tags:
    ...
    start = 0
    while True:
        url = "http://movie.douban.com/j/search_subjects?type=movie&tag=" + tag + "&page_limit=20&page_start=" + str(
            start)
        request = requests.get(url)
        movies = json.loads(request.text)['subjects']
        if len(movies) == 0:
            break
        for item in movies:
            rate = item['rate']
